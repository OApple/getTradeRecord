...
void DataInitInstance::insert_follow_user(string users,vector<string>&userlist,vector<CTraderSpi*>& vac)
{
    ...
    {//num follow user
        ...
        NiuTraderSpi* ba= new NiuTraderSpi(*this,tmpacc[0],(tmpacc[1]));
    ...
    while (getline(myfile, str))
    {
        ...
        else
        {
            ...
            if(name_value.size()!=2)
            {
                ...
                _broker_id = name_value[1];
            }
            else if ("beginDate" == name_value[0]) {
                _begin_date = name_value[1];
            } else if ("endDate" == name_value[0]) {
                _end_date = name_value[1];
            }
...
void DataInitInstance::saveThostFtdcInvestorPositionFieldToDb(CThostFtdcInvestorPositionField *pInvestorPosition)
{
    ...
    //    cout <<"position sql="<<positonsql<<endl;
...
{
    int total_trade_num=0;
    int trade_days_num=0;
    double win_rate=0;
    double profit_loss_rate=0;
    int profit_num=0;
    int loss_num=0;
    ...
    string positonsql="update ctp_account set "+
            ...
            ",loss_num="+lexical_cast<string>(loss_num)+
             ",trade_days_num="+lexical_cast<string>(trade_days_num)+
            ",total_income="+lexical_cast<string>(total_income)+
...
void DataInitInstance:: saveSettlementToDB(NiuTraderSpi &TraderSpi)
{
    ...
    if(TraderSpi.close_num())
        win_rate=(double)TraderSpi.profit_num()/TraderSpi.close_num();
    if(TraderSpi.loss()!=0)
        profit_loss_rate=abs((double)TraderSpi.profit()/TraderSpi.loss());
    total_income=TraderSpi.total_income();
    profit_num=TraderSpi.profit_num();
    loss_num=TraderSpi.loss_num();
    max_single_income=TraderSpi.max_single_income();
    total_commission=TraderSpi.total_commission();
    AccountID=  TraderSpi.getInvestorID();
    string positonsql="update ctp_account set "+
            string("total_trade_num= ")+lexical_cast<string>(total_trade_num)+
            ",win_rate="+lexical_cast<string>(win_rate)+
            ",profit_loss_rate="+lexical_cast<string>(profit_loss_rate)+
            ",single_max_income="+lexical_cast<string>(max_single_income)+
            ",profit_num="+lexical_cast<string>(profit_num)+
            ",loss_num="+lexical_cast<string>(loss_num)+
              ",trade_days_num="+lexical_cast<string>(trade_days_num)+
            ",total_income="+lexical_cast<string>(total_income)+
             ",commission="+lexical_cast<string>(total_commission)+
            "  where ctp_account='"+(AccountID)+"'";
    mysqlpp::ScopedConnection con(*mysql_pool, true);
    mysqlpp::Query query = con->query();
    try
