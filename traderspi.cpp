...
void CTraderSpi:: ReqQrySettlementInfo()
{
    ...
    while(true)
    {
        ...
        if(_begin>_end)
        {
            ...
            LOG(ERROR)  << "last: " << _investorID<< endl;
        ...
        while(1)
        {
            ...
            LOG(INFO) << "<<<<----- ReqQrySettlementInfo TradingDay:"<<req.TradingDay<<" investorID= "<<_investorID<<((iResult == 0) ? "success" : "failed result=  ")<<iResult << endl;
            if(iResult)
                boost::this_thread::sleep(boost::posix_time::seconds(1));
            else
            {
...
{
    LOG(INFO) << "----->>>OnRspQrySettlementInfo" << endl;
...
void CTraderSpi::SaveTransactionRecord()
{
    ...
    BOOST_FOREACH(string&xline,line)
    {
        ...
        if(j==1)
        {
            ...
            {
                string cdate=string(xline,datefound+3);
...
void CTraderSpi::OnRtnTrade(CThostFtdcTradeField *pTrade)
{
    ...
    return;
    string  investorID=boost::lexical_cast<string>(pTrade->InvestorID);
    string brokerID=boost::lexical_cast<string>(pTrade->BrokerID);
...
double CTraderSpi::total_income() const
{
    return _total_income;
}

void CTraderSpi::setTotal_income(double total_income)
{
    _total_income = total_income;
}
int CTraderSpi::trade_days_num() const
{
    return _trade_days_num;
}

void CTraderSpi::setTrade_days_num(int trade_days_num)
{
