...
//    }
    ...
    string selectsql="select user_id from ctp_account where ctp_account='"+_investorID+"';";
    mysqlpp::Query query = con->query(selectsql);

    mysqlpp::StoreQueryResult ares = query.store();
    if(ares)
    {
        try{
            _user_id=lexical_cast<string>(ares[0]["user_id"]);
        }
        catch(...)
        {
            _user_id="0";
    ...
    //    _begin=date(2018,10,1);
    _begin= from_undelimited_string(dii._begin_date);
//    _end=date(time(NULL));
    if(dii._end_date.empty())
        _end=boost::gregorian::day_clock::local_day();
    else
        _end= from_undelimited_string(dii._end_date);
    //    d_iter=day_iterator(_begin);
}
...
void NiuTraderSpi::ReqUserLogin()
{
    ...
    strcpy(req.BrokerID, dii._broker_id.c_str());
...
void NiuTraderSpi:: ReqQrySettlementInfo()
{
    ...
    while(true)
    {
        ...
        {
            mysqlpp::ScopedConnection con(*dii.mysql_pool, true);
            string selectsql="select sum(Lots) from  ctp_transaction_record where investorid='"+_investorID+"';";
            mysqlpp::Query query = con->query(selectsql);
            ...
            {
                try{
                    _total_trade_num=lexical_cast<int>(ares[0]["sum(Lots)"]);
                }
                catch(...)
                {
                    _total_trade_num=0;
            ...
            query.reset();
            query<<"select count(*) from ctp_transaction_record where RealizedPL>0 and investorid='"+_investorID+"';";
            ares = query.store();
            if(ares)
            {
                try{
                    _profit_num=lexical_cast<int>(ares[0]["count(*)"]);
                }
                catch(...)
                {
                    _profit_num=0;
            ...
            query.reset();
            query<<"select count(*) from ctp_transaction_record where OC not in ('开') and investorid='"+_investorID+"';";
            ares = query.store();
            if(ares)
            {
                try{
                    _close_num=lexical_cast<int>(ares[0]["count(*)"]);
                }
                catch(...)
                {
                    _close_num=0;
            ...
            query.reset();
            query<<"select sum(RealizedPL) from ctp_transaction_record where RealizedPL>0 and investorid='"+_investorID+"';";
            ares = query.store();
            if(ares)
            {
                try
                {
                    _profit=lexical_cast<double>(ares[0]["sum(RealizedPL)"]);
                }
                catch(...)
                {
                    _profit=0;
            ...
            {
                try{
                    _loss=lexical_cast<double>(ares[0]["sum(RealizedPL)"]);
                }
                catch(...)
                {
                    _loss=0;
            ...
            if(ares)
            {try{
                    _loss_num=lexical_cast<int>(ares[0]["count(*)"]);
                }
                catch(...)
                {
                    _loss_num=0;
            ...
            query.reset();
            query<<"select sum(RealizedPL) from ctp_transaction_record where  investorid='"+_investorID+"';";
            ares = query.store();
            if(ares)
            {try{
                    _total_income=lexical_cast<double>(ares[0]["sum(RealizedPL)"]);
                }
                catch(...)
                {
                    _total_income=0;
            ...
            query.reset();
            query<<"select count(distinct close_date)  as trade_num from ctp_transaction_record where investorid='"+_investorID+"';";
            ares = query.store();
            if(ares)
            {try{
                    _trade_days_num=lexical_cast<int>(ares[0]["trade_num"]);
                }
                catch(...)
                {
                    _trade_days_num=0;
            ...
            query.reset();
            query<<"select sum(fee)   from ctp_transaction_record where investorid='"+_investorID+"';";
            ares = query.store();
            if(ares)
            {try{
                    _total_commission=lexical_cast<double>(ares[0]["sum(fee)"]);
                }
                catch(...)
                {
                    _total_commission=0;
            ...
            query.reset();
            query<<"select max(realizedpl)  as maxv from ctp_transaction_record where investorid='"+_investorID+"';";
            ares = query.store();
            if(ares)
            {try{
                    _max_single_income=lexical_cast<double>(ares[0]["maxv"]);
                }
                catch(...)
                {
                    _max_single_income=0;
                }
            }

            LOG(WARNING)  << "last: " << _investorID<< endl;
            dii.saveSettlementToDB(*this);
            return;
        }
        string idate=to_iso_string(_begin);
        strcpy(req.TradingDay, idate.c_str());
        while(1)
        {
            int iResult = _pUserApi->ReqQrySettlementInfo(&req, ++_requestID);
            LOG(WARNING) << "<<<<----- ReqQrySettlementInfo TradingDay:"<<req.TradingDay<<" investorID= "<<_investorID<<((iResult == 0) ? "success" : "failed result=  ")<<iResult << endl;
            if(iResult)
                boost::this_thread::sleep(boost::posix_time::seconds(1));
            else
            {
...
double NiuTraderSpi::total_income() const
{
    return _total_income;
}

void NiuTraderSpi::setTotal_income(double total_income)
{
    _total_income = total_income;
}
int NiuTraderSpi::trade_days_num() const
{
    return _trade_days_num;
}

void NiuTraderSpi::setTrade_days_num(int trade_days_num)
{
    _trade_days_num = trade_days_num;
}
double NiuTraderSpi::max_single_income() const
{
    return _max_single_income;
}

void NiuTraderSpi::setMax_single_income(double max_single_income)
{
    _max_single_income = max_single_income;
}
double NiuTraderSpi::total_commission() const
{
    return _total_commission;
}

void NiuTraderSpi::setTotal_commission(double total_commission)
{
    _total_commission = total_commission;
}
...
void NiuTraderSpi::SaveTransactionRecord()
{
    ...
    BOOST_FOREACH(string&xline,line)
    {
        ...
        if(j==1)
        {
            std::size_t datefound = xline.find("：");
            if (datefound!=std::string::npos)
            {
                cdate=string(xline,datefound+3,8);
                date create= from_undelimited_string(cdate);
                if(create+days(1)<_begin)
                {
                    LOG(ERROR)  << "jump: " << _investorID<<cdate<< endl;
                    return;
                }
                LOG(ERROR)  << "insert:" << _investorID<<cdate<< endl;
            }
        }
        if(j==12)
        {
            ...
            if (datefound!=std::string::npos)
            {
                ...
                std::size_t datefound2 = xline.find(".");
                string cdate=string(xline,datefound+3,datefound2-datefound);
                balancebf=trim_copy(cdate);
            }
        ...
        if(j==13)
        {
            ...
            if (datefound!=std::string::npos)
            {
                ...
                std::size_t datefound2 = xline.find(".");
                string cdate=string(xline,datefound+3,datefound2-datefound);
                deposit_withdrawal=trim_copy(cdate);
            }

            datefound = xline.find_last_of("：");
            if (datefound!=std::string::npos)
            {

                std::size_t datefound2 = xline.find_last_of(".");
                string cdate=string(xline,datefound+3,datefound2-datefound);
                balancecf=trim_copy(cdate);
            }
    ...
    string sql;
    sql = "replace INTO qh_user_everyday_data (user_id,balance ,pre_balance,close_profit,commission,settle_date,create_date ,is_trade,deposit )  VALUES (";
    sql.append("'"+_user_id+"'"+",");
    sql.append(balancecf+",");
    sql.append(balancebf+",");
    sql.append(realizedpl+",");
    sql.append(commission+",");
    sql.append(cdate+",");
    sql.append(cdate+",");
    sql.append(is_trade+",");
    sql.append(deposit_withdrawal+");");
    mysqlpp::ScopedConnection con(*dii.mysql_pool, true);
    mysqlpp::Query query = con->query();
    try
    {
